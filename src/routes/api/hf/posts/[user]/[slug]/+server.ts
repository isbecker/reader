...
export const GET: RequestHandler = async ({ request, fetch, params }) => {
	...
	const hfhtml = await fetch(url).then(async (res) => await res.text());
	...
	const element = document.querySelector(
		'.SVELTE_HYDRATER[data-target="SocialPost"]',
	);
	...
	if (element) {
		...
		if (dataProps) {
			...
			const socialPost = PostParser.safeParseSocialPost(
				decodedProps.socialPost,
			);
